...
bool SDL_RenderPresent(SDL_Renderer *renderer)
{
    ...
    // SUSPECT 1: The GPU Command Flush
    ...
    // SUSPECT 3: SDL's Artificial CPU Sleeper
    uint64_t t5 = SDL_GetPerformanceCounter();
    ...
    uint64_t t6 = SDL_GetPerformanceCounter();

    double flushMs = (double)(t2 - t1) / freq * 1000.0;
    double presentMs = (double)(t4 - t3) / freq * 1000.0;
    double simVsyncMs = (double)(t6 - t5) / freq * 1000.0;

    // Only print if this specific function takes longer than ~5ms
    if (flushMs + presentMs + simVsyncMs > 5.0) {
        printf("SDL_RenderPresent: Flush=%.2fms | D3D11=%.2fms | SimVSync=%.2fms\n", 
                flushMs, presentMs, simVsyncMs);
    }
    return true;
}
